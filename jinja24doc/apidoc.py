                ...
                if ismethod(it) or isfunction(it) or ismethoddescriptor(it) or isgetsetdescriptor(it):
                    try:
                        ...
                        doc.append((mtype,                                  # type
                            uni(item.__name__) + '.' + uni(it.__name__),    # name
...
def keywords(api, api_url = "",                 # jinja function
    ...
    G.re_docs = re.compile(r"(\b)(%s)(\b)" % \
                '|'.join((name.replace('.','\.') for type, name, args, doc in api if type in types )))
    if G.re_docs.pattern == r"(\b)()(\b)":
        G.re_docs = None                # no keywrods found
    G._api_url = api_url

    G._api_keywords = dict((name, (args if type != 'module' else None) or type) for type, name, args, doc in api)
